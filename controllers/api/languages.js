...
router.get("/data_table", authorize(ADMIN), async (req, res) => {
  ...
  const queryObj = Language.find(query)
  ...
  const recordsFiltered = await Language.find(query).count();
  const recordsTotal = await Language.find({ isDeleted: false }).count();
...
router.get("/", async (req, res) => {
  const language = await Language.find({ isDeleted: false });
  res.sendApiResponse({ data: language });
});

router.post("/", authorize(ADMIN), async (req, res) => {
  const { value, error } = validateLanguage(req.body);
  if (error)
  ...
  const language = await new Language(value).save();
  res.sendApiResponse({ data: language });
});
