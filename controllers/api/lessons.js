...
const {bulkVideo,documentUpload}=require("../../middlewares/upload")
...
router.get("/remove_doc/:id",authorize(ADMIN),async(req,res)=>{
const {id}=req.params;
...
return res.sendApiResponse({data:lesson})
});

router.post(
  "/attach_doc/:id", authorize(ADMIN),
  documentUpload.single("doc"),
  async (req, res) => {
    const { id } = req.params;
    const { file, body } = req;
    const { error, value } = validateQuizDocument(body);
    if (error) {
      return res.sendApiResponse({
        status: 400,
        message: error.details[0].message,
    ...
    if (!file) {
      return res.sendApiResponse({ message: "Please upload PDF file." });
    }
    if (file) {
      ...
      value.doc = uploadResult.Location;
      console.log("value  ", value);
    }
    const quizPdf = await Lesson.findOneAndUpdate(
      {
        _id: id,
        isDeleted: false,
      },
      {
        $set: {
          "quiz.pdfName": value.pdfName,
          "quiz.desc": value.desc,
          "quiz.doc": value.doc,
        },
      },
      { new: true }
    ...
    return res.sendApiResponse({ data: quizPdf });
  }
);
router.post("/", authorize(ADMIN), bulkVideo.any("video"), async (req, res) => {
  const { error, value } = validateBulkLesson(req.body);
  if (error) {
    return res.sendApiResponse({
      status: 400,
      message: error.details[0].message
  ...
  const { files } = req;
  if (!files) {
    return res.sendApiResponse({status:400, message: "Please upload video files." });
  }
  try {
    for (let element of files) {
      const dotIndex = element.location.lastIndexOf(".");
      const fullLength = element.location.length;
      const substrDiff = fullLength - dotIndex;
      const newStr = substrDiff + 36;
      const fileLoc = element.location;
      const rawVideo = fileLoc.slice(-newStr);
      const newVideo = rawVideo.substring(0, rawVideo.lastIndexOf("."));
      const name = newVideo;
      await new Lesson({
        chapter,
        rawVideo,
        newVideo,
        name,
        videoQualities:["1024","720","480","360"],
        isBulk: true,
      }).save();
    }
  } catch (error) {
...
router.put("/:id", authorize(ADMIN), async (req, res) => {
  ...
  const lesson = await Lesson.findOneAndUpdate({ _id: id }, value, {
    new: true,
  });
  if (!lesson) {
    return res.sendApiResponse({
...
//   const { files } = req;
//   if (!files) {
//     return res.sendApiResponse({ message: "Please upload video files." });
//   }
//   for (let element of files) {
//     const fileLoc = element.location;
//     const rawVideo = fileLoc.slice(61);
//     const video = rawVideo.substring(0, rawVideo.lastIndexOf("."));
//     const name = video;
//     await new Lesson({ chapter, rawVideo, video, name }).save();
//   }
//   res.send("Videos Uploaded Successfully.");
// });
...
router.put(
  ...
  async (req, res) => {
    ...
    const prevLessonWithVideo = await Lesson.findOne({ newVideo: videoId });

    let videoQualities = ["360", "480", "720","1024"];
    ...
    const updatedLesson = await Lesson.findByIdAndUpdate(
      lessonId,
      {
        $set: { newVideo: videoId, videoQualities },
      },
      { new: true }
    );
