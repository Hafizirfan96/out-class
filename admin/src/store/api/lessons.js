...
//pdf
export const createLessonPdf = ({ id = "", body = {}, onSuccess, onError, onEnd } = {}) => {
  return async (dispatch, getState) => {
    const storeState = getState();
    const { token } = storeState.auth;
    try {
      //showing loader on ui
      console.log("bodywwwww---->",body)
      const res = await http.post({
        url: path +"/attach_doc/"+id,
        body,
        headers: {
          "x-auth-token": token,
          //  'content-type': 'multipart/form-data' 
...
//pdf
export const deleteLessonPdf = ({ id = "", onSuccess, onError, onEnd } = {}) => {
  return async (dispatch, getState) => {
    const storeState = getState();
    const { token } = storeState.auth;
    try {
