...
class EditLoginImage extends Component {
  ...
  loadCData = () => {
    ...
    this.props.loadCommonData({
      onSuccess: (res) => {
        const imagePreview = res.data.data.loginModalImage || "";
  ...
  onFormSubmit = (e) => {
    ...
    this.props.editLoginModalImage({
      id,
      body: formData,
      onEnd: () => {
        this.setState({ showLoader: false });
          ...
          <Card className="shadow">
            <CardHeader>
              <div className="clearfix">
                <h2 className="mb-0 float-left">Login Modal Image</h2>
              </div>
            </CardHeader>
...
const mapDispatchToprops = (dispatch) => ({
  editLoginModalImage: (params) => dispatch(editLoginModalImage(params)),
