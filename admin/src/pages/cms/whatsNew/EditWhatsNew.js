...
import {
  ...
  createStat,
  editSliderImage
} from "../../../store/api/whatsNew";
const schema = {
  heading: Joi.string().optional().allow(""),
  // text: Joi.string().optional().allow(""),
  buttonName: Joi.string().optional().allow(""),
  link: Joi.string().optional().allow(""),
};

class EditWhatsNew extends Component {
  state = {
    id: "",
    pages: [],
    form: {
      ...
      <form onSubmit={this.onFormSubmit}>
        <div className="row">
          ...
          {/* <div className="col-md-6">
            ...
            <br />
            <label>
              <span className="btn btn-info">Mobile Image</span>
              <input
                type="file"
                accept="image/*"
                multiple={false}
                className="d-none"
                onChange={(e) => {
                  const mobileImageSource = e.target.files[0];
                  if (mobileImageSource) {
                    this.setState({ mobileImageSource });
                    const reader = new FileReader();
                    reader.readAsDataURL(mobileImageSource);
                    reader.onloadend = function () {
                      this.setState({ mobileImagePreview: reader.result });
                    }.bind(this);
                  }
                }}
              />
          ...
          <Card className="shadow">
            <CardHeader>
              <div className="clearfix">
                <h2 className="mb-0 float-left">Edit Whats New Slider</h2>
              </div>
            </CardHeader>
...
export default connect(mapStateToProps, mapDispatchToprops)(EditWhatsNew);
